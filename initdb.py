...
datastrings = OrderedDict({
        ...
        "Games":"(Id INTEGER PRIMARY KEY AUTOINCREMENT, White INTEGER, Black INTEGER, Status INTEGER, Substatus INTEGER, Position CHAR(70), AwaitingPromote BIT(1), DeferredClaim INTEGER, ClaimIs3x BIT(1), ClaimFaultPlayer CHAR(1), ClaimFaults INTEGER, OfferRecipient INTEGER, FOREIGN KEY(Black) REFERENCES Users(Id), FOREIGN KEY(White) REFERENCES Users(Id), FOREIGN KEY (Status) REFERENCES GameStatuses(Id), FOREIGN KEY (Substatus) REFERENCES GameSubstatuses(Id), FOREIGN KEY(OfferRecipient) REFERENCES Users(Id), FOREIGN KEY(DeferredClaim) REFERENCES Users(Id))",
